...
def _save_outputs(
    *,
    ...
    safe_mode = ef_mode.replace("+", "plus")
    csv_path = os.path.join(out_dir, f"ef_{safe_mode}_{model_name}_loss_curve_bw{bitwidth}.csv")
    png_path = os.path.join(out_dir, f"ef_{safe_mode}_{model_name}_loss_curve_bw{bitwidth}.png")

    rows = [(step, losses[step - 1]) for step in range(1, len(losses) + 1)]

    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["step", f"loss_{safe_mode}"])
    ...
    plt.plot(steps, vals, color="#1f77b4", alpha=0.45, linewidth=1.3)
    plt.plot(
        steps,
        _moving_average(vals, window),
        color="#1f77b4",
        linewidth=2.0,
        label=f"{safe_mode} (MA{window})",
    )
    plt.xlabel("Step")
    plt.ylabel("Loss")
    plt.title(f"{model_name} EF mode={safe_mode} | bitwidth={bitwidth} | world_size={world_size}")
...
def _save_throughput_summary(
    *,
    ...
    os.makedirs(out_dir, exist_ok=True)
    safe_mode = ef_mode.replace("+", "plus")
    csv_path = os.path.join(out_dir, f"ef_{safe_mode}_throughput_summary_bw{bitwidth}.csv")
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Multi-model e2e benchmark for error-feedback modes")
    parser.add_argument("--ef-mode", type=str, default="none")
    ...
    ef_mode = str(args.ef_mode).strip().lower()
    allowed_modes = {"none", "legacy", "ef21", "ef21_plus", "ef21+", "no_compression"}
    if ef_mode not in allowed_modes:
        raise ValueError(f"Unsupported ef-mode: {args.ef_mode}")
    if ef_mode == "ef21+":
        ef_mode = "ef21_plus"
    ...
    bitscom.init(bitwidth=args.bitwidth, error_feedback_mode=ef_mode)
    ...
    try:
        for model_name in args.models:
            ...
            if rank == 0:
                print(
                    f"[bench] mode={ef_mode} model={model_name} steps={train_cfg['steps']} "
            ...
            if model_name == "resnet50":
                ...
                model = _build_bert_model(args.bert_num_labels, args.bert_hidden, args.bert_layers, args.bert_heads)
            ...
            run = _run_train(
                ...
                mode="no_compression" if ef_mode == "no_compression" else ef_mode,
                model=model,
                dataset=dataset,
                train_cfg=train_cfg,
                args=args,
            ...
            if rank == 0:
                throughput_rows.append(
                    (
                        model_name,
                        ef_mode,
                        float(run["avg_step_time_ms"]),
                        float(run["throughput_samples_per_s"]),
                    )
                )
                _save_outputs(
                    model_name=model_name,
                ...
                print(
                    f"[throughput] {model_name} mode={ef_mode} "
                    f"step_ms={run['avg_step_time_ms']:.3f} "
                    f"samples/s={run['throughput_samples_per_s']:.3f}"
                )
